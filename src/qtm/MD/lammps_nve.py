...
def write_lammps_data(filename, l_atoms, primvec, coords_cryst_all, bohr_to_angstrom):
    ...
    conv= np.linalg.inv(new_primvec)@primvec
    s = (conv@coords_cryst_all.T).T
    ...
    coords_cart_orig= np.dot(coords_cryst_all, primvec.T)
    ...
    with open(filename, 'w') as f:
        ...
        #f.write(f"0.0 {a_x:.6f} xlo xhi\n")
        #f.write(f"0.0 {b_y:.6f} ylo yhi\n")
        #f.write(f"0.0 {c_z:.6f} zlo zhi\n")
...
def lammps_NVE(dftcomm: DFTCommMod,
           crystal: Crystal,
           max_t: float,
           dt: float,
           T_init: float,
    ...
    with dftcomm.image_comm as comm:
        ...
        if comm.rank==0:
            ...
            write_lammps_data("data.lmp", l_atoms, reallat.primvec, coords_cryst_all, bohr_to_angstrom)
            ...
            print("Velocities extracted from LAMMPS (Angstrom/ps", v, "in processor", comm.rank, flush=True)  # Print first 5 for brevity
            ...
            ke = 0.5 * np.sum(mass_all * np.sum(vel**2, axis=1))  # Kinetic energy in Hartree
            ...
            for step in range(time_steps):
                ...
                coords_cart_all = x*reallat.alat

                  # Convert to bohr
                print(f"Step {step}: Coordinates from LAMMPS in bohr alat units: {coords_cart_all} in processor {comm.rank}", flush=True)
                #coords_cart_all = reallat.alat*coords_cart_all  # Convert
                print(f"Step {step}: Coordinates from LAMMPS: {coords_cart_all} in processor {comm.rank}", flush=True)
            

                # Compute energy and forces using DFT
                en, force_coord, rho_md = compute_en_force(dftcomm, coords_cart_all, rho_md)
            ...
            comm.Bcast(coords_cart_all, root=0)
